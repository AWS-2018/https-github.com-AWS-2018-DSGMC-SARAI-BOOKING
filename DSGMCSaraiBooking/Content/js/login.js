...
function validate_login_form(form, data) {
    if (data.txtUserName == "") {
        ...
        addFormError(form["txtUserName"], 'The username is invalid');
        return false; // stop the script if validation is triggered
    }

    if (data.txtPassword == "") {
        // if password variable is empty
        addFormError(form["txtPassword"], 'The password is invalid');
        return false; // stop the script if validation is triggered
    }
...
form_handler("#FrmLogin", validate_login_form, null, null, null, null, null, null);
